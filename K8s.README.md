...
Nota: para powershell usar este comando (para que genere el .yml en utf8 y no de error el helm install):
```
 kubectl create deployment client-gateway --image=<registro/url/imagen> --dry-run=client --output=yaml | Out-File .\templates\client-gateway\deployment.yml -Encoding UTF8
```
